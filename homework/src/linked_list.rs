...
impl<T> IterMut<'_, T> {
    ...
    pub fn insert_next(&mut self, element: T) {
        ...
        node.next = self.head;
        if self.head.is_null() {
            // self.tail要么是空指针，要么指向list最后一个节点
            node.prev = self.tail;
            let node = Box::into_raw(Box::new(node));
            unsafe { self.tail.as_mut() }.map(|tail| tail.next = node);
        } else {
            ...
            unsafe { node.as_mut() }.map(|n| {
                if !n.prev.is_null() {
                    unsafe { n.prev.as_mut() }.map(|p| p.next = node);
                }
                else {
                    // 若node.prev为空指针，说明node是list的第一个节点，则需要更新list.head
                    self.list.head = node;
                }
            });
        }
